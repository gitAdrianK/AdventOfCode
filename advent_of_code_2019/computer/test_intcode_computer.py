...
class TestStringMethods(unittest.TestCase):
    ...
    def test_addition(self):
        ...
        computer.initialize_memory(self.regex.findall("1,0,0,0,99"))
        computer.run()
        self.assertEqual([2, 0, 0, 0, 99], computer.memory)

    def test_multiplication(self):
        computer = IntCodeComputer()
        computer.initialize_memory(self.regex.findall("2,3,0,3,99"))
        computer.run()
        self.assertEqual([2, 3, 0, 6, 99], computer.memory)

    def test_large_multiplication(self):
        computer = IntCodeComputer()
        computer.initialize_memory(self.regex.findall("2,4,4,5,99,0"))
        computer.run()
        self.assertEqual([2, 4, 4, 5, 99, 9801], computer.memory)

    def test_addition_multiplication(self):
        computer = IntCodeComputer()
        computer.initialize_memory(self.regex.findall("1,1,1,4,99,5,6,0,99"))
        computer.run()
